...
class AccountData extends ChangeNotifier {
  ...
  Future<void> _init() async {
    ...
    String? raw = prefs.getString('ultimate_report_v190');
  ...
  void loadMonth(String month) {
    ...
    final incKeys = ['기본급','장기근속수당','시간외근무수당','가족수당','식대보조비','대우수당','직무수행급','성과급','성과금','임금인상분','기타1','기타2'];
    income = {for (var k in incKeys) k: (d['income']?[k] ?? 0)};

    final dedKeys = ['갑근세','주민세','건강보험료','고용보험료','국민연금','요양보험','식권구입비','노동조합비','환상성금','아동발달지원계좌','교양활동반회비','기타1','기타2','기타3'];
    deduction = {for (var k in dedKeys) k: (d['deduction']?[k] ?? 0)};

    final fixKeys = ['KB보험','삼성생명','주택화재보험','한화보험','변액연금','일산','암사동','주택청약','사촌모임회비','용돈'];
    fixedExp = {for (var k in fixKeys) k: (d['fixedExp']?[k] ?? (k=='KB보험'?133221:k=='삼성생명'?167226:k=='주택화재보험'?24900:k=='한화보험'?28650:k=='변액연금'?200000:k=='일산'?300000:k=='암사동'?300000:k=='주택청약'?100000:k=='사촌모임회비'?30000:k=='용돈'?500000:0))};

    final varKeys = ['십일조','대출원리금','연금저축','IRP','식비','교통비','관리비','도시가스','하이패스','통신비'];
    variableExp = {for (var k in varKeys) k: (d['variableExp']?[k] ?? 0)};

    final chiKeys = ['교육비(똘1)','교육비(똘2)','주식(똘1)','주식(똘2)','청약(똘1)','청약(똘2)','교통비(똘1)','교통비(똘2)'];
    childExp = {for (var k in chiKeys) k: (d['childExp']?[k] ?? 0)};
  ...
  void _save() async {
    ...
    final prefs = await SharedPreferences.getInstance();
    prefs.setString('ultimate_report_v190', jsonEncode(storage));
  }
...
class _MainScaffoldState extends State<MainScaffold> with SingleTickerProviderStateMixin {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        centerTitle: true,
        title: _tab.index >= 3 ? Text(_tab.index == 3 ? "12개월 리포트" : "저축 대시보드") : ActionChip(
...
// 수입/지출 리스트 (항목 누락 방지 및 잘림 방지 패딩 적용)
        ...
        child: SizedBox(
          ...
          child: TextField(
            ...
            decoration: InputDecoration(
              labelText: k, labelStyle: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.black87),
...
class TabCard extends StatelessWidget { const TabCard({super.key}); @override Widget build(BuildContext context) { final d = context.watch<AccountData>(); String ld = ""; bool sh = false; return Column(children: [Expanded(child: Scaffold(floatingActionButton: FloatingActionButton.small(onPressed: () => _cardEditDlg(context, d, null), child: const Icon(Icons.add)), body: ListView.separated(itemCount: d.cardLogs.length, separatorBuilder: (ctx, i) => const Divider(height: 1), itemBuilder: (ctx, i) { final log = d.cardLogs[i]; if (log['date'] != ld) { sh = !sh; ld = log['date']; } return Container(color: sh ? Colors.orangeAccent.withOpacity(0.12) : Colors.white, child: ListTile(dense: true, title: Text("${log['date'].substring(5)} | ${log['desc']} (${log['card']})"), subtitle: (log['note'] != null && log['note'] != "") ? Text(log['note'], style: const TextStyle(fontSize: 10)) : null, trailing: Text("${d.nf.format(log['amt'])}원", style: const TextStyle(fontWeight: FontWeight.bold)), onTap: () => _cardEditDlg(context, d, i), onLongPress: () { d.cardLogs.removeAt(i); d.notifyListeners(); })); }))), _summaryBox([_row("카드 합계", d.cardLogs.fold(0, (a, b) => a + (b['amt'] as int)), Colors.indigo, b: true)])]); } }

// 3. 통계: 막대그래프 제거 후 텍스트 기반 12개월 리포트 구현
  ...
  @override Widget build(BuildContext context) {
    ...
    return Column(children: [
      if (!d.isStatsViewMode) ...[
        ...
        Padding(padding: const EdgeInsets.all(16.0), child: ElevatedButton(onPressed: () => setState(() { d.isStatsViewMode = true; confirmedItems = Set.from(d.tempCheckedItems); }), child: const Text("리포트 생성하기"))),
      ] else ...[
        Padding(padding: const EdgeInsets.all(16.0), child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [const Text("📋 최근 12개월 리포트", style: TextStyle(fontWeight: FontWeight.bold, fontSize: 16)), IconButton(icon: const Icon(Icons.refresh), onPressed: () => setState(() => d.isStatsViewMode = false))])),
        Expanded(child: ListView.builder(
          ...
          itemBuilder: (context, i) {
            ...
            return Card(
              ...
              child: ListTile(
                leading: CircleAvatar(backgroundColor: Colors.orange.shade100, child: Text("${targetDate.month}월", style: const TextStyle(fontSize: 12, fontWeight: FontWeight.bold))),
                title: Text("${targetDate.year}년 ${targetDate.month}월", style: const TextStyle(fontSize: 12)),
                trailing: Text("${d.nf.format(sum.toInt())}원", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.indigo)),
              ),
            );
          },
        )),
...
class TabSaving extends StatelessWidget {
  ...
  Widget _bar(String u, int cur, int g, Color c) {
    ...
    return Column(children: [
      ...
      SizedBox(width: 140, child: LinearProgressIndicator(value: p, minHeight: 20, borderRadius: BorderRadius.circular(12), color: c, backgroundColor: c.withOpacity(0.1))),
...
void _cardEditDlg(BuildContext context, AccountData d, int? idx) {
  ...
  String desc = log?['desc'] ?? ""; int amt = log?['amt'] ?? 0; String brand = log?['card'] ?? "우리"; String dateStr = log?['date'] ?? DateFormat('yyyy-MM-dd').format(DateTime.now()); String note = log?['note'] ?? "";
  showDialog(context: context, builder: (ctx) => StatefulBuilder(builder: (ctx, setS) => AlertDialog(
    title: Text(isEdit ? "카드 내역 수정" : "카드 내역 추가"),
    content: SingleChildScrollView(child: Column(mainAxisSize: MainAxisSize.min, children: [
      ListTile(title: Text(dateStr), trailing: const Icon(Icons.calendar_month), onTap: () async {
      ...
      TextField(controller: TextEditingController(text: note), decoration: const InputDecoration(labelText: "비고"), onChanged: (v) => note = v),
    ])),
    actions: [
      ...
      TextButton(onPressed: () {
        ...
        else d.addCardLog(desc, amt, brand, DateFormat('yyyy-MM-dd').parse(dateStr), note);
        Navigator.pop(ctx);
...
void _savingDlg(BuildContext context, AccountData d, String u) {
  ...
  showDialog(context: context, builder: (ctx) => AlertDialog(title: Text("$u 저축 입력"), content: TextField(keyboardType: TextInputType.number, decoration: const InputDecoration(suffixText: "원"), onChanged: (v) => amt = int.tryParse(v) ?? 0), actions: [TextButton(onPressed: () { d.addSaving(u, amt, date); Navigator.pop(ctx); }, child: const Text("저장"))]));
}
